...
export function JupiterEvolutionModal({ isOpen, onClose, onSuccess }: JupiterEvolutionModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className={`bg-[#121212] w-full max-w-md rounded-[32px] border border-white/10 shadow-2xl overflow-hidden transition-all duration-500`}>
                <div className="p-8">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={onClose} disabled={step === 'loading'} className="p-2 hover:bg-white/5 rounded-full text-gray-400 transition-colors disabled:opacity-50">
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    {step === 'auth' && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <p className="text-sm text-gray-400 mb-8 leading-relaxed">
                                Conecte-se ao JúpiterWeb para mapear automaticamente as disciplinas que você <strong className="text-emerald-400">já concluiu</strong> e as que <strong className="text-amber-400">está cursando</strong>.
                            </p>
                    ...
                    {(step === 'loading' || step === 'review') && (
                        <div className="animate-in fade-in zoom-in-95 duration-500 space-y-6">
                            <div className="max-h-[70vh] min-h-[50vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10 space-y-6">
                                ...
                                {step === 'loading' ? (
                                    <>
                                        <div className="space-y-3">
                                            ...
                                            {[1, 2, 3, 4, 5, 6].map(i => (
                                                <div key={`skel-c-${i}`} className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/5">
                                    ...
                                    <>
                                        <div className="space-y-3">
                                            ...
                                            {concluidas.length === 0 ? (
                                                 <p className="text-sm text-gray-600 italic px-2">Nenhuma detectada nesta extração.</p>
                                            ) : (
                                                <div className="grid grid-cols-2 gap-2">
                                                    {concluidas.map(code => (
                                                        <div key={code} className="flex items-center gap-2 p-2.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                                                            ...
                                                            <span className="text-xs font-bold text-emerald-100 tracking-widest">{code}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
