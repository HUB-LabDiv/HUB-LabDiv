                ...
                <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                     <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10">
                        ...
                        <div className="flex flex-col">
                            <h3 className="text-xs font-black uppercase text-white tracking-widest">{targetProfile.full_name || targetProfile.name}</h3>
                    ...
                    <div className="h-[600px] bg-white/5 rounded-3xl border border-white/5 overflow-hidden">
        ...
        if (activeGroup) {
            return (
                <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="h-[600px] flex flex-col bg-white/5 rounded-3xl border border-white/5 overflow-hidden">
                        <div className="flex-1 overflow-y-auto p-6 space-y-4" ref={groupChatRef}>
                            {groupMessages.map(msg => (
                                <div key={msg.id} className="flex flex-col">
                                    ...
                                    <div className="bg-white/5 p-4 rounded-2xl rounded-tl-none border border-white/5 max-w-[85%] text-[13px] text-gray-100 leading-relaxed shadow-sm">
                                        {msg.content}
                                    </div>
                        ...
                        <div className="p-4 border-t border-white/5 bg-black/20 flex gap-2">
                             <input 
                                ...
                                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-brand-yellow/50 transition-all" 
                             />
        ...
        return (
            <div className="space-y-12 pb-20">
                ...
                <div className="flex flex-col items-center text-center max-w-lg mx-auto">
                    ...
                    <h2 className="text-2xl font-black text-white uppercase tracking-tighter mb-2">Nexus de Emaranhamento</h2>
                    ...
                    <div className="w-full mt-10 relative group">
                         ...
                         <input 
                            ...
                            className="w-full pl-14 pr-4 py-5 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.15em] text-white outline-none focus:border-brand-blue/50 focus:bg-white/10 transition-all placeholder:text-gray-700" 
                         />
                    ...
                    {searchResults.length > 0 && (
                        <div className="w-full mt-4 space-y-2 max-h-[250px] overflow-y-auto pr-2 scrollbar-hide">
                            {searchResults.map(user => (
                                <button key={user.id} onClick={() => setTargetProfile({ id: user.id, full_name: user.full_name, avatar_url: user.avatar_url })} className="w-full flex items-center gap-3 p-4 bg-white/5 hover:bg-brand-blue/20 rounded-2xl border border-white/5 transition-all group text-left">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[11px] font-black uppercase text-white group-hover:text-brand-blue transition-colors">{user.full_name}</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {recommendedGroups.map(group => (
                                <button key={group.id} onClick={() => openGroupChat(group)} className="flex items-center gap-4 p-5 bg-brand-yellow/5 border border-brand-yellow/10 rounded-3xl hover:bg-brand-yellow/10 hover:border-brand-yellow/30 transition-all text-left group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h5 className="text-[12px] font-black text-white uppercase tracking-tight truncate">{group.name}</h5>
                     ...
                     <div className="space-y-6">
                        ...
                        {isGroupsLoading ? (
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {myGroups.map(g => (
                                    <button key={g.id} onClick={() => openGroupChat(g)} className="w-full p-5 bg-white/5 hover:bg-brand-yellow/5 border border-white/5 hover:border-brand-yellow/20 rounded-3xl transition-all flex items-center gap-4 text-left group">
                                        ...
                                        <div className="flex-1 overflow-hidden">
                                            <div className="text-[11px] font-black uppercase text-white truncate mb-0.5">{g.name}</div>
                            ...
                            <div className="py-12 flex flex-col items-center justify-center text-center px-6 bg-white/5 rounded-[32px] border border-dashed border-white/10 opacity-50">
                        ...
                        {isRecentLoading ? (
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {recentConversations.map(conv => (
                                    <button key={conv.id} onClick={() => setTargetProfile({ id: conv.id, full_name: conv.name, avatar_url: conv.avatar })} className="w-full p-5 bg-white/5 hover:bg-brand-blue/5 border border-white/5 hover:border-brand-blue/20 rounded-3xl transition-all flex items-center gap-4 text-left group">
                                        {conv.avatar ? <img src={conv.avatar} className="size-11 rounded-2xl object-cover" /> : <div className="size-11 rounded-2xl bg-brand-blue/10 flex items-center justify-center text-brand-blue font-black uppercase text-sm border border-brand-blue/10">{conv.name[0]}</div>}
                                        <div className="flex-1 overflow-hidden">
                                            <div className="text-[11px] font-black uppercase text-white truncate mb-0.5">{conv.name}</div>
        ...
        <div className="max-w-5xl mx-auto py-8 px-4">
            ...
            {showCreateGroup && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4 animate-in fade-in duration-300">
                     <div className="bg-[#181818] border border-white/10 rounded-[40px] w-full max-w-md p-10 space-y-8 shadow-2xl relative">
                        <button onClick={() => setShowCreateGroup(false)} className="absolute right-8 top-8 text-gray-500 hover:text-white transition-colors p-2 hover:bg-white/5 rounded-full"><X className="size-6" /></button>
                        ...
                        <div className="text-center space-y-2">
                             ...
                             <h3 className="text-lg font-black uppercase text-white tracking-widest">Estruturar Núcleo</h3>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-xs text-white outline-none focus:border-brand-yellow/50 focus:bg-white/10 transition-all uppercase placeholder:text-gray-700" 
                                />
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative group">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-14 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase text-white outline-none focus:border-brand-yellow/50 focus:bg-white/10 transition-all placeholder:text-gray-700" 
                                    />
                                ...
                                {memberSearchResults.length > 0 && (
                                    <div className="max-h-[150px] overflow-y-auto mt-2 space-y-1 pr-1 scrollbar-hide animate-in slide-in-from-top-2">
                                        {memberSearchResults.map(m => (
                                            <button key={m.id} onClick={() => { setSelectedMembers(prev => [...prev, m]); setMemberSearchQuery(''); setMemberSearchResults([]); }} className="w-full flex items-center gap-3 p-3 bg-white/5 hover:bg-brand-yellow/10 border border-white/5 rounded-xl text-left group">
                                                ...
                                                <span className="text-[10px] font-black text-white uppercase group-hover:text-brand-yellow transition-colors">{m.full_name}</span>
                                ...
                                <div className="flex flex-wrap gap-2 mt-4">
                                    {selectedMembers.map(m => (
                                        <div key={m.id} className="flex items-center gap-2 px-3 py-2 bg-brand-yellow/10 text-brand-yellow rounded-xl text-[9px] font-black uppercase border border-brand-yellow/20 animate-in zoom-in-50">
                                            {m.full_name.split(' ')[0]}
                                            <button onClick={() => setSelectedMembers(prev => prev.filter(p => p.id !== m.id))} className="hover:text-white transition-colors"><X className="size-3" /></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
