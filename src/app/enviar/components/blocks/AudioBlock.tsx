...
export default function AudioBlock({ block, isActive }: { block: Block; isActive: boolean }) {
    ...
    return (
        <div className="flex flex-col gap-4">
            {!audioUrl ? (
                <div className="w-full min-h-[12rem] border-2 border-dashed border-blue-500/30 rounded-xl flex flex-col items-center justify-center text-blue-400/70 transition-colors bg-blue-900/10 p-6">
                    ...
                    {isActive && (
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button 
                                ...
                                className="px-3 py-1 bg-brand-red text-white text-xs rounded-lg font-medium shadow-lg hover:bg-red-600 transition-colors"
                            >
                                Remover
                            </button>
                        </div>
