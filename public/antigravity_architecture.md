...
### Passo 6: Precisão do Aviso Offline & Controle Granular de Cache (Logins & Rascunhos)
*   **Aviso de Offline Preciso:** O aviso de desconexão informa com precisão o estado real do cache (se no modo total, apenas interações em páginas já navegadas são mantidas; se no modo restrito, apenas logins e rascunhos são mantidos).
*   **Controle do Modo Offline-First & Restrito:** No Gerenciador de Arquivos/Armazenamento (`/configuracoes` -> Armazenamento & Cache), o usuário pode selecionar o nível de cache:
    *   **Offline-First Total:** Cache de páginas navegadas, mídias, login e rascunhos.
    *   **Restrito (Apenas Login & Rascunhos):** Desativa o cache automático de mídias e páginas no navegador, mantendo **estritamente protegidos** os dados de autenticação (sessão de login) e rascunhos de formulários.
    *   **Desativado:** Sem pré-carregamento em segundo plano.

### Passo 7: Tratamento de Conflitos e Feedback Avançado de UI
